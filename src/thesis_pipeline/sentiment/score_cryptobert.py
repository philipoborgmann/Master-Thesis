"""CryptoBERT sentiment scoring — wraps Sentiment_score_cryptobert.py."""
...
def run(*, smoke: bool = False, max_rows: int | None = None,
    ...
    outputs = [resolve_path("sentiment_scored_cryptobert"),
               resolve_path("cryptobert_checkpoints")]
    ...
    if dry_run:
        get_logger().info("dry-run: not invoking Sentiment_score_cryptobert.py")
        return 0

    argv: list[str] = [
        "--input",  str(resolve_path("sentiment_combined")),
        "--output", str(resolve_path("sentiment_scored_cryptobert")),
    ]
    ...
    if restart:
        argv += ["--restart"]
    return run_script_main("score_cryptobert", argv)
